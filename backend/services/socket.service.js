...
function connectSockets(http, session) {
    ...
    gIo.on('connection', socket => {
        ...
        socket.on('looking', topic => {
            ...
            if (clients > 1) {
                gIo.to(socket.myTopic).emit('message', `${clients} Other people are watching this event`)
            }
            else if (clients === 0) {
                gIo.to(socket.myTopic).emit('message', `Be the first to join this event!`)
            }
            else null
